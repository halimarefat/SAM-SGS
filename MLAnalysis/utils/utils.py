...
M1_HEADERS = ['Ux', 'Uy', 'Uz', 'S1',  'S2', 'S3', 'S4', 'S5', 'S6', 'Nut']
M2_HEADERS = ['G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'S1',  'S2', 'S3', 'S4', 'S5', 'S6', 'Nut']
M3_HEADERS = ['Ux', 'Uy', 'Uz', 'UUp1',  'UUp2', 'UUp3', 'UUp4', 'UUp5', 'UUp6', 'Nut']
M4_HEADERS = ['G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'UUp1',  'UUp2', 'UUp3', 'UUp4', 'UUp5', 'UUp6', 'Nut']
M5_HEADERS = [#'Ux', 'Uy', 'Uz', 
              'G1', 'G2', 'G3', 'G4', 'G5', 'G6', 
              'S1',  'S2', 'S3', 'S4', 'S5', 'S6', 
...
def DataCollecter():
    with open(f'{MOTHERDIR}/datasets/fieldData_means.txt', 'r') as file:
        data = [float(line.strip()) for line in file]
    means = pd.DataFrame(np.reshape(data, (-1, len(HEADERS))), columns=HEADERS)

    with open(f'{MOTHERDIR}/datasets/fieldData_scales.txt', 'r') as file:
        data = [float(line.strip()) for line in file]
    scales = pd.DataFrame(np.reshape(data, (-1, len(HEADERS))), columns=HEADERS)

    norm = pd.read_csv(f'{MOTHERDIR}/datasets/fieldData_norm.txt', sep=' ', names=HEADERS)
    org = pd.read_csv(f'{MOTHERDIR}/datasets/fieldData_noheader.csv', sep=' ', names=HEADERS)
