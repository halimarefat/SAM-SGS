...
from utils.utils import OFLESDataset, R2Score, DataCollecter, MOTHERDIR
from utils.utils import OFLESDataset, R2Score, M2_HEADERS, M5_HEADERS
...
num_epochs = 40 #800
patience = 30
best_model_path = f'{MOTHERDIR}/best_model.pt'
out_channels = 1
in_channels = dt.shape[1] - out_channels
split_sz = 0.8
batch_sz_trn = 4096 * 10
...
# Load the model if a previous checkpoint exists
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        traced_script_module.save(f"./traced_model.pt")
...
print(f"\n Training history saved to '{MOTHERDIR}/training_history.json'")
...
print(f"Traced model saved to '{MOTHERDIR}/traced_model.pt'")
