...
volScalarField eddyViscosityField(const fvMesh& mesh)
{
    // Read the eddy viscosity field from the mesh
    volScalarField nut
    (
        IOobject
        (
...
{   
    ...
    outPtr.reset(new OFstream(outPath/"fieldData_.dat"));
    ...
    forAll(timeDirs, timeI)
    {
      ...
      if (runTime.timeName() != "0")
      {
        ...
        volScalarField nut = eddyViscosityField(mesh);
        ...
        for (label cellI = 0; cellI < mesh.C().size(); cellI++)
        {
          ...
          if (cellCenter.x() > 1.0 && cellCenter.x() < 9.0 &&
              cellCenter.y() > 0.0 && cellCenter.y() < 3.0 &&
              cellCenter.z() > 0.0 && cellCenter.z() < 0.15)
          {          
